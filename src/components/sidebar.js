...
async function prefetchPage(pageId) {
  ...
  const lastPrefetchedAt = _prefetchedAt.get(pageId);
  if (lastPrefetchedAt && (Date.now() - lastPrefetchedAt) < PREFETCH_TTL_MS) return;
...
const SORT_LABELS = {
  manual: 'navigation.sort.manual',
  name: 'navigation.sort.name',
  created: 'navigation.sort.created',
  changed: 'navigation.sort.changed',
};
...
function getSortConfig(parentId) {
  try {
    const storedSortValue = localStorage.getItem(SORT_KEY(parentId)) || 'manual';
    let [mode, dir] = storedSortValue.split(':');
...
function applySort(pages, sortConfig) {
  ...
  switch (mode) {
    case 'name':
      return sortedPages.sort((a, b) => mult * (a.title || '').localeCompare(b.title || '', 'de', { sensitivity: 'base' }));
    case 'created':
      return sortedPages.sort((a, b) => mult * (tsMs(a.createdAt) - tsMs(b.createdAt))); // default desc handled by getSortConfig
    case 'changed':
      return sortedPages.sort((a, b) => mult * (tsMs(a.updatedAt) - tsMs(b.updatedAt))); // default desc handled by getSortConfig
    case 'manual':
    default:
      return sortedPages.sort((a, b) => mult * ((a.order || 0) - (b.order || 0)));
  }
}
...
function openOptionsMenu(anchorEl, { parentId, page }) {
  ...
  if (page && canEdit()) {
    ...
    newChild.textContent = i18next.t('navigation.newSubpage');
    newChild.addEventListener('click', async () => {
      closeMenu();
      const title = await promptModal(i18next.t('messages.newSubpageTitle'), i18next.t('messages.newSubpagePlaceholder'));
    ...
    del.textContent = i18next.t('common.delete');
    del.addEventListener('click', async () => {
      closeMenu();
      const confirmed = await confirmModal(i18next.t('editor.deletePage'), i18next.t('messages.deletePageConfirm', { title: page.title || i18next.t('common.untitled') }));
      if (!confirmed) return;
      try {
    ...
    if (currentSortMode === 'manual') {
      ...
      moveUp.innerHTML = `<span style="margin-right: 8px;">↑</span> ${i18next.t('navigation.moveUp')}`;
      ...
      moveDown.innerHTML = `<span style="margin-right: 8px;">↓</span> ${i18next.t('navigation.moveDown')}`;
  ...
  heading.textContent = parentId ? i18next.t('navigation.sortSubpages') : i18next.t('navigation.sortRoot');
  ...
  for (const mode of SORT_MODES) {
    ...
    const label = i18next.t(SORT_LABELS[mode]) + (mode === 'manual' ? '' : ` ${arrow}`);
...
function createTreeItem(page, allFilteredPages) {
  ...
  name.textContent = page.title || i18next.t('common.untitled');
  ...
  if (sortConfig.mode !== 'manual' && hasChildren && !searchFilter) {
    ...
    sortIndicator.title = i18next.t('navigation.sortedBy', { mode: i18next.t(SORT_LABELS[sortConfig.mode]) });
...
function renderTrashHeader(container) {
  ...
  header.innerHTML = `
    ...
    <span>${i18next.t('navigation.trash')}</span>
  ...
  if (trashExpanded) {
    ...
    list.innerHTML = `<div class="trash-loading">${i18next.t('common.loading')}</div>`;
...
async function renderTrash(container) {
  ...
  try {
    ...
    if (deletedPages.length === 0) {
      ...
      empty.textContent = i18next.t('messages.trashEmpty');
    ...
    topLevel.forEach((page) => {
      ...
      if (canEdit()) {
        ...
        restoreBtn.title = i18next.t('editor.restore');
        ...
        deleteBtn.title = i18next.t('editor.permanentDelete');
        ...
        deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const confirmed = await confirmModal(i18next.t('editor.permanentDelete') + '?', i18next.t('messages.deletePermanentConfirm'));
          if (!confirmed) return;
          try {
    ...
    errMsg.textContent = i18next.t('common.error');
