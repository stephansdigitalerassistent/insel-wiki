...
test.describe('Live Insel-Wiki E2E Tests', () => {
  ...
  async function login(page) {
    ...
    if (didLogin) {
        // Playwright starts with empty cache. If we just logged in, the sidebar listener might have died with permission-denied.
        // Reloading ensures all listeners start with the cached auth state.
        await page.reload();
        await page.waitForLoadState('domcontentloaded');
    }
  ...
  test.afterEach(async ({ page }) => {
    ...
    // Soft Delete any created pages that still exist
    for (const id of createdPageIds) {
      await page.goto(`/#/${id}`);
      await page.waitForTimeout(1000); // wait for page load
      const titleInput = page.locator('#page-title');
      if (await titleInput.isVisible() && await titleInput.inputValue() !== '') {
          // It exists, delete it
  ...
  test('Core functionality: Pages, Tasks, Search', async ({ page }) => {
    ...
    // We don't strictly require the class to be .tree-item if the live app is old
