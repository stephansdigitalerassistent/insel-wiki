...
test.describe('Comprehensive Feature Tests', () => {
  ...
  test('User Profile Modal: open and close', async ({ page }) => {
    ...
    await expect(profileModal).not.toHaveClass(/hidden/, { timeout: 10000 });
  ...
  test('Comments Sidebar: add comment and close', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      await expect(commentsSidebar).not.toHaveClass(/hidden/, { timeout: 3000 });
    }).toPass({ timeout: 15000 });
    
    await expect(commentsSidebar).toBeVisible({ timeout: 5000 });
